...
class EuropeanCallPrime(Equation):
    def __init__(self, dim, total_time, num_time_interval, sigma, r, K):
        super(EuropeanCallPrime, self).__init__(dim, total_time, num_time_interval)
        self._sigma = sigma
        self._r = r
        self._K = K
#         self.lower, self.upper = ob_range
    ...
    def sample(self, num_sample):
#        x_init = np.random.uniform(self.lower, self.upper, [ num_sample, self.dim ])
        x_init = np.ones([ num_sample, self.dim ]) * 100.0
        dw_sample = np.random.randn(num_sample,
                                     self.dim,
                                     self.num_time_interval) * self._sqrt_delta_t
        x_sample = np.zeros([num_sample, self.dim, self.num_time_interval + 1])
